...
export default function TripBuildScreen() {
  ...
  useEffect(() => {
    // When user selects a fixture, set dates to match-weekend-ish defaults (no scrolling required)
    ...
    // Nudge the list slightly so user sees the selection + panel (avoids “did anything happen?”)
  ...
  function openPicker(which: "start" | "end") {
    if (Platform.OS === "web" || !DateTimePicker) {
      // Web (and no-picker fallback): keep ISO text inputs below
      return;
    }
  ...
  function onPickerChange(_: any, date?: Date) {
    // Android fires "dismiss" with undefined date; iOS may keep open.
    if (!date) {
  ...
  const panelHeight = panelOpen ? 260 : 0;
            ...
            {!loading && !error && filteredRows.length === 0 ? (
              <View style={{ marginTop: 12 }}>
                <EmptyState
                  ...
                  message="Try a different league, date window, or search."
                />
              </View>
            ) : null}
            ...
            {!panelOpen ? (
              <View style={{ marginTop: 14 }}>
                <Text style={styles.hint}>
                  Tip: Tap a fixture to open trip details and save immediately.
                </Text>
              </View>
            ) : null}
          </GlassCard>
        ...
        {panelOpen ? (
          <View style={styles.panelWrap} pointerEvents="box-none">
            <GlassCard style={styles.panel} intensity={22}>
              ...
              {/* Web + no-datetimepicker fallback: ISO inputs (still stored correctly) */}
            ...
            {DateTimePicker && picker.open ? (
              <DateTimePicker
                value={
                  parseIsoDate(picker.which === "start" ? startIso : endIso) ?? new Date()
                }
                mode="date"
