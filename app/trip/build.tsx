...
export default function TripBuildScreen() {
  ...
  const [endDate, setEndDate] = useState(addDaysIso(from, 2));
  ...
  useEffect(() => {
    // When user selects a fixture, set dates to match-weekend-ish defaults
    const iso = selectedFixture?.fixture?.date as string | undefined;
    if (!iso) return;

    const start = toIsoDateInput(new Date(iso));
    setStartDate(start);
    setEndDate(addDaysIso(start, 2));
  }, [selectedFixture]);
      ...
      <SafeAreaView style={styles.container} edges={["bottom"]}>
        ...
        >
          <GlassCard style={styles.card}>
            <Text style={styles.h1}>1) Pick a match</Text>
            <Text style={styles.muted}>
              Choose a league, then pick a fixture to anchor the trip.
            </Text>
            ...
            {!loading && !error && rows.length === 0 ? (
              <View style={{ marginTop: 12 }}>
                <EmptyState
                  ...
                  message="Try a different league or date window."
                />
              </View>
            ) : null}
          ...
          <GlassCard style={styles.card}>
            <Text style={styles.h1}>2) Trip details</Text>
            <Text style={styles.muted}>
              Basic fields for v1. We’ll make this richer once Trips UX is locked.
            </Text>
            ...
            <View style={styles.field}>
              ...
              <TextInput
                ...
                placeholder="Hotel idea, train times, pubs, mates, etc."
                placeholderTextColor={theme.colors.textSecondary}
            ...
            <Text style={styles.smallPrint}>
              ...
              {selectedFixture?.fixture?.id
                ? String(selectedFixture.fixture.id)
                : "none"}
            </Text>
          </GlassCard>
        </ScrollView>
