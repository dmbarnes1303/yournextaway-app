...
async function openMatch(matchId: string) {
  ...
  // SE365 deep link from fixture enrichment
  const baseEventUrl = (r as any)?.se365EventUrl;

  if (!baseEventUrl) {
    Alert.alert("Tickets not available");
    return;
  }

  // append affiliate id if provided
  const aff = process.env.EXPO_PUBLIC_SE365_AFFILIATE_ID;
  const url =
    aff && !baseEventUrl.includes("aff=")
      ? `${baseEventUrl}${baseEventUrl.includes("?") ? "&" : "?"}aff=${aff}`
      : baseEventUrl;
