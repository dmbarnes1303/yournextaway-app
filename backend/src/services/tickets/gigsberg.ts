...
function buildEventUrl(eventId: string): string {
  ...
  publicBase.searchParams.set("query", eventId);
...
export async function resolveGigsbergCandidate(
  ...
  if (!listings.length) {
    const fallbackUrl = appendAffiliate(buildListingUrl(bestEventName || `${homeName} vs ${awayName}`));
    ...
    return {
      ...
      url: fallbackUrl || buildEventUrl(bestEventId),
      title: `Tickets: ${getPreferredTeamName(input.homeName)} vs ${getPreferredTeamName(input.awayName)}`,
      priceText: null,
  ...
  const listingUrl = appendAffiliate(buildListingUrl(bestEventName || `${homeName} vs ${awayName}`));
